...
/*TODO: we might need to update the featureRomTimeStamp too, skip for now
...
*/
	...
	*slot = DEFAULT_PL_PS_SLOT;
	if (xocl_axlf_section_header(xdev, axlf, BITSTREAM) ||
		...
		int reqlen = 0;
                struct xcl_mailbox_req *req_ptr = NULL;
                int rspret = -ETIME;
                size_t rsplen = sizeof(rspret);
		
		printk("DZ_ here here\n");

		if (!(xdev->core.vmgmt_subdev.vmgmt_mbx_platdev &&
			XOCL_GET_DRV_PRI(xdev->core.vmgmt_subdev.vmgmt_mbx_platdev) &&
			XOCL_GET_DRV_PRI(xdev->core.vmgmt_subdev.vmgmt_mbx_platdev)->ops)) {
			userpf_err(xdev, "%d", __LINE__);
			err = -EFAULT;
			goto out_done;
		}

                reqlen = struct_size(req_ptr, data, 1) + sizeof(uuid_t);
                req_ptr = vmalloc(reqlen);
                if (!req_ptr) {
                        err = -ENOMEM;
                        goto out_done;
                }

                userpf_info(xdev, "try to load xclbin_uuid: %pUb",
                        (xuid_t *)&bin_obj.m_header.uuid);

                req_ptr->req = XCL_MAILBOX_REQ_LOAD_XCLBIN_UUID;
                downloaded_xclbin = (xuid_t *)&bin_obj.m_header.uuid;
                memcpy(req_ptr->data, downloaded_xclbin, sizeof(uuid_t));

                mbx_ops = XOCL_GET_DRV_PRI(xdev->core.vmgmt_subdev.vmgmt_mbx_platdev)->ops;
                rc = mbx_ops->request(xdev->core.vmgmt_subdev.vmgmt_mbx_platdev, req_ptr, reqlen,
                                      &rspret, &rsplen, NULL, NULL, 0, 0);
                vfree(req_ptr);
                if (rc) {
                        userpf_err(xdev, "mailbox reqest err %d", rc);
                        err = -EINVAL;
                        goto out_done;
                }

                if (rspret) {
                        userpf_err(xdev, "load xclbin ret %d", rspret);
                        err = rspret;
                        goto out_done;
                }

                userpf_info(xdev, "UUID of loaded XCLBIN: %pUb",
                        (xuid_t *)&bin_obj.m_header.uuid);
	...
	if (dtbHeader) {
		 ...
		 */
		/*TODO: current xclbin is lite version, no need to check uuid. */
		//if (xocl_axlf_section_header(xdev, axlf, BITSTREAM)) {
	...
	if (!(xdev->core.vmgmt_subdev.vmgmt_icap_platdev &&
		XOCL_GET_DRV_PRI(xdev->core.vmgmt_subdev.vmgmt_icap_platdev) &&
		XOCL_GET_DRV_PRI(xdev->core.vmgmt_subdev.vmgmt_icap_platdev)->ops)) {
			userpf_err(xdev, "%d", __LINE__);
			err = -EINVAL;
			goto out_done;
	}
	...
	icap_ops = XOCL_GET_DRV_PRI(xdev->core.vmgmt_subdev.vmgmt_icap_platdev)->ops;
	err = icap_ops->download_bitstream_axlf(xdev->core.vmgmt_subdev.vmgmt_icap_platdev, axlf,
						*slot);
	...
	if (!err) {
		...
		if (err) {
			icap_ops->clean_bitstream(xdev->core.vmgmt_subdev.vmgmt_icap_platdev,
						  *slot);
		}
	}
done:
			 ...
			 struct drm_file *filp)
{
	...
	printk("karthik vmgmt=%d", XOCL_VMGMT_MBX_PROTOCOL_VERSION(xdev));
